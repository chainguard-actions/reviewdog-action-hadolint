...
wget -q "https://github.com/hadolint/hadolint/releases/download/${HADOLINT_VERSION}/${HADOLINT_FILE}" \
    -O "${TEMP_PATH}/hadolint" \
...
if [ -n "${INPUT_HADOLINT_IGNORE}" ]; then
  read -ra _rules <<< "${INPUT_HADOLINT_IGNORE}"
...
if [ -n "${INPUT_HADOLINT_FLAGS}" ]; then
  # Split INPUT_HADOLINT_FLAGS into array words safely
  read -ra HADOLINT_FLAGS <<< "${INPUT_HADOLINT_FLAGS}"
fi
HADOLINT_FLAGS+=("${IGNORE_FLAGS[@]}")
